...
from services.ssh_manager import connect_ssh
...
@server.route('/servers/<int:id>')
@login_required
def get_server(id):
    server_obj = Server.query.get(id)
    if not server_obj:
...
@server.route('/delete-server/<int:id>', methods=['POST', 'DELETE'])
@login_required
...
@server.route('/connect-server/<int:id>', methods=['POST'])
@login_required
